...
- **Persistence** – `/persist` reports queue status for the current chunk, `/save` forces an autosave pass, `/load [chunkX] [chunkZ]` preloads a chunk and waits for its persistence record, `/inspect chunk [chunkX] [chunkZ]` surfaces tracked metadata, `/wipe chunk [chunkX] [chunkZ]` clears local persistence state, and `/snapshot now [chunkX] [chunkZ]` immediately compacts the tracked snapshot. 【F:three-demo/src/player/dev-commands.js†L2937-L3112】
