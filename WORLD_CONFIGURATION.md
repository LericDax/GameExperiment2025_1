...
The descriptor now derives these clamp values from a three-chunk vertical span: with 48-voxel chunks, the ±144 bounds wrap a base plane parked at 10 voxels, leaving ample headroom for taller formations. The macro/detail/ridge amplitudes (≈84/13/10) apportion the 134-voxel budget toward broad landforms while keeping high-frequency detail restrained, preventing the pillar forests that appeared when the detail operator consumed the expanded headroom.【F:three-demo/src/world/world-option-descriptors.js†L56-L87】 The paired frequencies divide the preserved slope budgets (`0.48`, `0.24`, and `0.048`) by the live amplitudes, so amplitude·frequency always matches the historical envelope even if the defaults change again.【F:three-demo/src/world/world-option-descriptors.js†L89-L118】 Regression coverage samples neighbouring columns at runtime and fails once the 95th percentile slope exceeds the tightest legacy budget by 8%—`min(0.48, 0.24) × 1.08`—which mirrors the ≈0.215 plateau observed with the retuned defaults while still giving a narrow alerting buffer.【F:three-demo/src/world/__tests__/terrain-default-slope-budget.test.js†L68-L108】
