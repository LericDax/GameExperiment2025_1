...
The descriptor now derives these clamp values from a three-chunk vertical span: with 48-voxel chunks, the ±144 bounds wrap a base plane parked at 10 voxels, leaving ample headroom for taller formations. The macro/detail/ridge amplitudes (≈84/13/10) apportion the 134-voxel budget toward broad landforms while keeping high-frequency detail restrained, preventing the pillar forests that appeared when the detail operator consumed the expanded headroom.【F:three-demo/src/world/world-option-descriptors.js†L55-L86】 The paired frequencies back-solve the legacy slope budgets—`0.06 × 8`, `0.08 × 3`, and `0.02 × 2.4`—so the amplitude·frequency product matches the historical profile even if the default amplitudes shift.【F:three-demo/src/world/world-option-descriptors.js†L88-L113】 Regression coverage samples neighbouring columns at runtime and fails if the 95th percentile slope drifts above 0.26, roughly 20% above the observed 0.215 plateau, ensuring amplitude/frequency recalibrations stay in lock-step.【F:three-demo/src/world/__tests__/terrain-default-slope-budget.test.js†L68-L105】
