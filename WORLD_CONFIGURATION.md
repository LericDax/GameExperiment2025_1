...
The descriptor now derives these clamp values from a three-chunk vertical span: with 48-voxel chunks, the ±144 bounds wrap a base plane parked at 10 voxels, leaving ample headroom for taller formations. The macro/detail/ridge amplitudes (≈84/13/10) apportion the 134-voxel budget toward broad landforms while keeping high-frequency detail restrained, preventing the pillar forests that appeared when the detail operator consumed the expanded headroom.【F:three-demo/src/world/world-option-descriptors.js†L56-L96】 Each paired frequency now back-solves from the preserved slope budgets (`0.48`, `0.24`, `0.048`) by scaling the legacy carriers (`0.06`, `0.08`, `0.02`) with the ratio `legacyAmplitude / liveAmplitude` and the legacy-span factor (≈`48 / 144`). This keeps amplitude·frequency pinned to the historical envelope even as the live amplitudes respond to taller vertical spans.【F:three-demo/src/world/world-option-descriptors.js†L96-L132】【F:three-demo/src/world/world-settings.js†L214-L279】 Regression coverage samples columns at multiple spacings—including unit-spacing neighbour checks—and fails as soon as the 95th percentile slope reaches the tightest legacy budget (`min(0.48, 0.24)`), catching any future amplitude/frequency mismatches immediately.【F:three-demo/src/world/__tests__/terrain-tfms-attenuation.test.js†L426-L507】【F:three-demo/src/world/__tests__/terrain-default-slope-budget.test.js†L41-L106】
