...
| `terrain.verticalSpanChunks` | Vertical Span (±chunks) | Number of chunk heights available above and below the datum when deriving clamps and wave budgets. | `number` | `3` | `1` – `32` | `1` |
...
| `terrain.clamp.min` | Clamp Minimum | Lower clamp bound applied after noise sampling to prevent deep pits. | `number` | `-144` | `-4096` – `1024` | `1` |
...
| `terrain.primaryFrequency` | Primary Frequency | Base frequency for macro terrain variation. Lower values create large landforms. | `number` | `0.0019047619047619048` | `0.0001` – `1` | `0.0001` |
...
| `terrain.detailFrequency` | Detail Frequency | Frequency of secondary detail used to break up flat areas. | `number` | `0.006153846153846153` | `0.0001` – `2` | `0.0001` |
...
| `terrain.ridgeFrequency` | Ridge Frequency | Frequency controlling how often sharp ridgelines occur. | `number` | `0.0016` | `0.0001` – `1` | `0.0001` |
...
| `verticalSpanChunks` | Vertical Span (alias) | Legacy top-level alias mirroring the terrain vertical span chunk count. | `number` | `3` | `1` – `32` | `1` |
| `maxHeight` | Max Height (alias) | Legacy top-level alias mirroring the terrain max height for compatibility. | `number` | `144` | `1` – `1024` | `1` |

The descriptor now derives clamp and slope defaults from the configurable vertical span: with the shipping three-chunk span and 48-voxel chunks, the ±144 bounds wrap a base plane parked at 10 voxels, leaving ample headroom for taller formations. The macro/detail/ridge amplitudes (≈84/13/10) apportion the 134-voxel budget toward broad landforms while keeping high-frequency detail restrained, preventing the pillar forests that appeared when the detail operator consumed the expanded headroom.【F:three-demo/src/world/world-option-descriptors.js†L56-L132】 Each paired frequency back-solves from the preserved slope budgets (`0.48`, `0.24`, `0.048`) by scaling the legacy carriers (`0.06`, `0.08`, `0.02`) with the ratio `legacyAmplitude / liveAmplitude` and the legacy-span factor (≈`48 / 144`). This keeps amplitude·frequency pinned to the historical envelope even as the live amplitudes respond to taller or shorter vertical spans.【F:three-demo/src/world/world-settings.js†L174-L309】 Regression coverage samples columns at multiple spacings—including unit-spacing neighbour checks—and fails as soon as the 95th percentile slope reaches the tightest legacy budget (`min(0.48, 0.24)`), catching any future amplitude/frequency mismatches immediately.【F:three-demo/src/world/__tests__/terrain-tfms-attenuation.test.js†L426-L507】【F:three-demo/src/world/__tests__/terrain-default-slope-budget.test.js†L41-L106】
