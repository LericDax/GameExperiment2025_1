...
function createMutableWorldOptions() {
  ...
  options.terrain.detailAmplitude = waveDefaults.detailAmplitude
  options.terrain.ridgeStrength = waveDefaults.ridgeStrength
  options.terrain.primaryFrequency = waveDefaults.primaryFrequency
  options.terrain.detailFrequency = waveDefaults.detailFrequency
  options.terrain.ridgeFrequency = waveDefaults.ridgeFrequency
...
function createDefaultTerrainTfmsPreset(terrainDefaults) {
  ...
  const safePrimaryAmplitude = Number.isFinite(baseAmplitude)
    ...
    : 1
  const safeDetailAmplitude = Number.isFinite(detailAmplitude)
  ...
  const normalizedPrimaryForStrength =
    normalizedBaseAmplitude * primaryInverseRatio
  const normalizedDetailForStrength =
    normalizedDetailAmplitude * detailInverseRatio
  const primaryStrengthNormalizationRatio =
    normalizedPrimaryForStrength > 0
      ? normalizedPrimaryForStrength / primaryBudgetReference
      : 1
  ...
  const detailStrengthNormalizationRatio =
    normalizedDetailForStrength > 0
      ? normalizedDetailForStrength / detailBudgetReference
      : 1
  ...
  const legacyPrimaryBudgetRatio =
    normalizedPrimaryForStrength > 0
      ? clampValue(
          LEGACY_TFMS_PRIMARY_AMPLITUDE / normalizedPrimaryForStrength,
          0,
          1,
        )
      : 1
  const legacyDetailBudgetRatio =
    normalizedDetailForStrength > 0
      ? clampValue(
          LEGACY_TFMS_DETAIL_AMPLITUDE / normalizedDetailForStrength,
          0,
          1,
        )
      : 1
  ...
  const derivedWarpBaseline =
    ...
    primaryStrengthInverseRatio *
    warpStrengthNormalization
  const derivedWarpPreClamp = derivedWarpBaseline * primaryInverseRatio
  let derivedWarp = clampValue(derivedWarpPreClamp, 0, 1)
  const derivedPhaseBaseline =
    ...
    primaryStrengthInverseRatio *
    warpStrengthNormalization
  const derivedPhasePreClamp = derivedPhaseBaseline * primaryInverseRatio
  let derivedPhase = clampValue(derivedPhasePreClamp, 0, 1)
  ...
  const domainWarpAmplitudeReference = 0.32 * primaryInverseRatio
  const domainWarpAmplitudeBaseline =
    Math.min(warpMultiplier, domainWarpAmplitudeReference) *
    warpNormalizationRatio
...
export function applyWorldOptions(overrides = {}) {
  ...
  if (resolvedBaseHeight !== null) {
    ...
    baseHeightChanged = baseHeight !== previousBaseHeight
