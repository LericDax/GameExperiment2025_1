...
function addTree(x, z, groundHeight) {
  ...
  for (let dx = -canopyRadius; dx <= canopyRadius; dx += 1) {
    for (let dy = -canopyRadius; dy <= canopyRadius; dy += 1) {
...
for (let x = -halfSize; x < halfSize; x += 1) {
  for (let z = -halfSize; z < halfSize; z += 1) {
    ...
    if (height < worldConfig.waterLevel) {
      for (let y = height + 1; y <= worldConfig.waterLevel; y += 1) {
...
function buildInstancedMeshes() {
  Object.entries(instancedData).forEach(([type, matrices]) => {
    ...
    mesh.castShadow = !['cloud', 'water'].includes(type);
...
function clampToWorld(position) {
  const boundary = worldConfig.chunkSize / 2 - 1;
